...
pub fn draw_text_centered(text: &str, y_offset: f32, size: u16, color: Color) {
    ...
    draw_text(text, screen_width()/2.0 - dims.width/2.0, screen_height()/2.0 - dims.height/2.0 + y_offset, size as f32, color);
}
...
pub fn draw_ship(ship: &Ship) {
    ...
    let color = if ship.rapid_fire_timer > 0.0 { PURPLE } else { WHITE };
